...
func Test_File(t *testing.T) {
	data := action.Store(action.Store_FileGroup_Target, "file/path", `
...
<staple -$---hU> <svg xmlns="http://www.w3.org/2000/svg" style="display: none;"> <defs> <filter id="glass-distortion" x="0%" y="0%" width="100%" height="100%"> <feTurbulence type="fractalNoise" baseFrequency="0.008 0.008" numOctaves="2" seed="92" result="noise" /> <feGaussianBlur in="noise" stdDeviation="2" result="blurred" /> <feDisplacementMap in="SourceGraphic" in2="blurred" scale="77" xChannelSelector="R" yChannelSelector="G" /> </filter> </defs> </svg> </staple> 
	...
	res := script.Rider(&data, []string{"class"}, types.Script_Action_Read)
	shell.Render.Raw(res)
	shell.Post(res.Scribed)
}
...
<staple -$---hU> <svg xmlns="http://www.w3.org/2000/svg" style="display: none;"> <defs> <filter id="glass-distortion" x="0%" y="0%" width="100%" height="100%"> <feTurbulence type="fractalNoise" baseFrequency="0.008 0.008" numOctaves="2" seed="92" result="noise" /> <feGaussianBlur in="noise" stdDeviation="2" result="blurred" /> <feDisplacementMap in="SourceGraphic" in2="blurred" scale="77" xChannelSelector="R" yChannelSelector="G" /> </filter> </defs> </svg> </staple> 
		`
	data := action.Store(action.Store_FileGroup_Target,
		"file/path",
		content,
		"target",
		"source",
	...
	cursor := Cursor.Construct(content)
	cursor.Increment()
	res := script.Tag_Scanner(&data, []string{"class"}, types.Script_Action_Read, &cursor)
	shell.Render.Raw(res)
	shell.Post(content[:res.StyleDeclarations.EndMarker])
}
