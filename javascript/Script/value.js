          ...
          if (["<", ">"].includes(entry[0])) {
            ...
            if (index) {
              switch (action) {
                case "watch":
                  const devClass =
                    metaFront + CACHE.Index2StylesObject[index].metaClass;
                  scribed += Use.string.normalize(
                    devClass,
                    ["/", ".", ":", "|", "$"],
                    ["\\"],
                  );
                ...
                case "publish":
                  if (activeIndexes.includes(index)) {
                    ...
                    const deltaStyles = Use.object.onlyB(
                      activeStyles,
                      CACHE.Index2StylesObject[index].object,
                    );
                    if (deltaStyles.score)
                      ...
                      fileData.usedIndexes.add(identity.number);
                      scribed += identity.class;
                      CACHE.Index2StylesObject[identity.number] =
                        deltaStyles.result;
