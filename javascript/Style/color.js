...
export function labToRgb(L, a, b) {
	...
	let b_linear = X_norm * 0.0557 + Y_norm * -0.204 + Z_norm * 1.057;
...
export function oklabToRgb(L, a, b) {
	...
	let b_linear =
		-0.0041960863 * l_linear - 0.7034186147 * m_linear + 1.707614701 * s_linear;
...
const regex = {
	...
	oklab:
		/\boklab\(\s*(-?\d*\.?\d+)\s+(-?\d*\.?\d+)\s+(-?\d*\.?\d+)(?:\s*\/\s*(-?\d*\.?\d+))?\s*\)/gi,
	oklch:
...
export default function LoadColorFallback(string = "") {
	...
	for (const method of targetMethods) {
		...
		if (!currentRegex) {
			// Corrected: If regex doesn't exist, skip.
		...
		result = result.replace(currentRegex, (match, ...args) => {
			...
			switch (method) {
				...
				default:
					console.warn(
						`Unhandled color method in loadColorFallback: ${method}`,
					);
			...
			if (
				convertedRgb &&
				typeof convertedRgb.r === "number" &&
				typeof convertedRgb.g === "number" &&
				typeof convertedRgb.b === "number"
			) {
				// Pass alpha if it was captured and is a valid number, otherwise default to 1
				const finalAlpha =
					alpha !== undefined && !isNaN(parseFloat(alpha))
						? parseFloat(alpha)
						: 1;
				return convert.LoadHex(
					convertedRgb.r,
					convertedRgb.g,
					convertedRgb.b,
					finalAlpha,
