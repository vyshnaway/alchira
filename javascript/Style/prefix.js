...
export function forSelector(
  ...
  const stringList = Use.string.zeroBreaks(content, [","]).map((i) => i.trim()),
    selectors = [];
  stringList.forEach((string = "") => {
    ...
    prefixes.forEach((group) => {
      result[group].out = string.replace(/:+[\w-]+/g, (selector) => {
        if (PREFIX.selector[selector] && PREFIX.selector[selector][group])
          result[group].score++;
        return (
          (PREFIX.selector[selector] && PREFIX.selector[selector][group]) ||
...
export function LoadProps(
  ...
  if (PREFIX.clrprops.includes(attribute)) {
    ...
    Object.values(attributes).forEach((attr) =>
      values.forEach((val) => results.push([attr, val])),
    );
  } else {
