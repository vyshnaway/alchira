...
export default class C_Target {
    constructor({ source, target, stylesheet, extensions, fileContents, stylesheetContent, }, label) {
        this.source = "";
        this.target = "";
        this.stylesheet = "";
    ...
    SaveFile(filePath, fileContent, fileIndex = Object.keys(this.fileCache).length) {
        ...
        const FILE = Filing("target", filePath, fileContent, this.target, this.source, `${this.label}_${Use.string.enCounter(fileIndex)}`);
        ...
        ParseResponse.stylesList.forEach((tagStyle) => {
            if (tagStyle.symclasses.length === 0) {
                ...
                const IndexMap = tagStyle.scope === _Style._Type.GLOBAL ? FILE.styleData.globalClasses
                    : tagStyle.scope === _Style._Type.LOCAL ? FILE.styleData.localClasses
                        : tagStyle.scope === _Style._Type.PUBLIC ? FILE.styleData.publicClasses
                            : {};
                const skeletonMap = tagStyle.scope === _Style._Type.LOCAL ? FILE.manifesting.local
    ...
    Accumulator() {
        ...
        Object.values(this.fileCache).forEach((file) => {
            ...
            if (localKeys.length + globalKeys.length + publicKeys.length) {
                Cumulates.report.push($.MAKE($.tag.H6(file.targetPath, $.preset.tertiary), 
                // [
    ...
    SyncClassnames(action) {
        Object.values(this.fileCache).forEach((filedata) => {
            filedata.scratch = NARRATOR(filedata, this.extnsProps[filedata.extension], action).stream;
        });
    }
