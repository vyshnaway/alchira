...
function SCANNER(content, initial, sourceSelector, merge_n_flatten) {
    ...
    const scannedAst = Object.fromEntries([
        ...
        ...Object.entries(scanned.atProps).map(([propKey, propValue]) => [propKey, CACHE.STATIC.DEBUG ? `${propValue}/* ${initial} ${sourceSelector} */` : propValue]),
        ...Object.entries(scanned.properties).map(([propKey, propValue]) => [propKey, CACHE.STATIC.DEBUG ? `${propValue}/* ${initial} ${sourceSelector} */` : propValue])
    ]);
...
function TagStyleScanner(raw, file, IndexMap = {}) {
    ...
    if (group !== _Style._Type.NULL) {
        ...
        const styleScanned = SCANNER(Use.code.uncomment.Script(raw.styles['']), `${_Style._Import[raw.scope]} : ${file.filePath} ||`, `${raw.symclasses} => []`, false);
        ...
        for (const subSelector in raw.styles) {
            if (subSelector !== "") {
                ...
                if (query.status) {
                    const styleScanned = SCANNER(Use.code.uncomment.Script(raw.styles[subSelector]), `${_Style._Import[raw.scope]} : ${file.filePath} |`, `${raw.symclasses} => ${subSelector}`, true);
                    attachments.push(...styleScanned.attachments);
                    Object.assign(variables, styleScanned.variables);
        ...
        const style_snippet = SCANNER(raw.elid === CACHE.ROOT.customElements.style ? Use.code.uncomment.Script(raw.attachstring) : '', `${_Style._Import[raw.scope]}:ATTACHMENT : ${file.filePath}:${raw.rowIndex}:${raw.colIndex} |`, `${raw.symclasses[0]}`, true);
