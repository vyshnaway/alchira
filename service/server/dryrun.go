...
func Dryrun(step Dryrun_Step_enum, watch bool) (status bool) {
	...
	for {
		...
		case Dryrun_Step_ProcessBlueprint:
			compiler.Update_Blueprint()
			fallthrough
			...
			if configs.Static.WATCH {
				...
				if Refer.watcher == nil {
					watch_dirs := append(
						...
						configs.Path_Folder["blueprint"].Path,
					)
					ignore_dirs := []string{
				...
				if Refer.watcher.Length() > 12 {
					...
					if event.Folder == configs.Path_Folder["blueprint"].Path {
						if event.Action == _watcher.E_Action_Update {
							...
							default:
								...
								step = Dryrun_Step_ProcessBlueprint
							}
						} else {
