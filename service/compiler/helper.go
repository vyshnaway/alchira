	...
	for i := range dependmap {
		if data := _action.Index_Fetch(i); data != nil {
			...
			if _config.Static.PREVIEW {
				attach_sketchs.WriteString(script.SketchCompile(i, script.E_Method_PreviewHash, map[int]bool{i: true}))
			} else {
				attach_sketchs.WriteString(script.SketchCompile(i, script.E_Method_DebugHash, map[int]bool{i: true}))
			}
		}
	}
