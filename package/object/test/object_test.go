...
func TestSetAndGet(t *testing.T) {
	m := O.New[string, int](5)
	m.Set("a", 1)
	m.Set("b", 3)
	m.Set("b", 2)
	m.Set("fg", 5)
	m.Set("", 2)
	console.Render.Raw(m.GetKeys())
	console.Render.Raw(m.GetVals())
...
func TestDelete(t *testing.T) {
	...
	expected := []string{"y"}
	if !reflect.DeepEqual(m.GetKeys(), expected) {
		t.Errorf("expected keys %v, got %v", expected, m.GetKeys())
	}
}
