...
func Sync_Bulk(source, target string, extInclude, extnUnsync, fileExcludes []string, sync bool, concurrent bool) (map[string]string, error) {
	if concurrent {
		return Sync_BulkParallel(source, target, extInclude, extnUnsync, fileExcludes, sync)
	} else {
		return Sync_BulkSerial(source, target, extInclude, extnUnsync, fileExcludes, sync)
	}
}

// Bulk synchronizes two directories, handling inclusions, exclusions, and unsynced extensions.
func Sync_BulkParallel(source, target string, extInclude, extnUnsync, fileExcludes []string, sync bool) (map[string]string, error) {
	concurrent := true
...
// Bulk synchronizes two directories, handling inclusions, exclusions, and unsynced extensions.
func Sync_BulkSerial(source, target string, extInclude, extnUnsync, fileExcludes []string, sync bool) (map[string]string, error) {
	concurrent := false
