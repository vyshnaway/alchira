...
const asymEncrypt = async (data, publicKey) => {
    const importedKey = await subtle.importKey("spki", Buffer.from(publicKey, "base64"), {
        name: "RSA-OAEP",
        hash: "SHA-256",
    }, false, ["encrypt"]);
    ...
    const encrypted = await subtle.encrypt({
        name: "RSA-OAEP",
    }, importedKey, encodedData);
    return Buffer.from(encrypted).toString("base64");
};
const asymDecrypt = async (encryptedData, privateKey) => {
    const importedKey = await subtle.importKey("pkcs8", Buffer.from(privateKey, "base64"), {
        name: "RSA-OAEP",
        hash: "SHA-256",
    }, false, ["decrypt"]);
...
async function symEncrypt(data, base64Key, base64Iv) {
    ...
    const encodedData = new TextEncoder().encode(data);
    const generatedKey = await subtle.importKey("raw", keyBuffer, {
        name: "AES-GCM",
...
function symDecrypt(encryptedData, base64Key, base64Iv) {
    ...
    const iv = Buffer.from(base64Iv, "base64");
    const encryptedBuffer = Buffer.from(encryptedData, "base64");
    const key = subtle.importKey("raw", keyBuffer, {
        name: "AES-GCM",
    }, false, // non-extractable
    ["decrypt"]);
...
//# sourceMappingURL=kryptic.js.map