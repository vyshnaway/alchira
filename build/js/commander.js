...
function reporter(heading, targets, report) {
    $.render.write($.MAKE("", [
        $.MAKE($.tag.H1(heading, $.preset.primary), targets.map(i => `Watching : ${i}`), [$.list.Bullets, 0, $.preset.tertiary]),
        report,
        $.MAKE($.tag.H4("Press Ctrl+C to stop watching.", $.preset.failed), CACHE.STATIC.Tweaks.CacheUsage ? ACTION.GetCacheUsage() : [], [$.list.Catalog, 0, $.preset.tertiary]),
    ]));
}
async function execute(chapter) {
    ...
    let OutFiles = {};
    let SaveAction = null;
    let report = "", targets = [], reportNext = false, step = "Initialize", staticsFetched = false, heading = "Initial Build";
    do {
        switch (step) {
            case "Initialize":
                        ...
                        if (event.action === "add" || event.action === "change") {
                            switch (pathFromWork) {
                                ...
                                case CACHE.PATH.json.hashrule.path:
                                    ...
                                    break;
                                default:
                                    if (pathFromWork.startsWith(CACHE.PATH.folder.libraries.path)
                                        && event.extension === "css") {
                                        CACHE.STATIC.Libraries_Saved[pathFromWork] = event.fileContent;
                                    }
                                    else if (pathFromWork.startsWith(CACHE.PATH.folder.artifacts.path)
...
async function commander({ command, argument, rootPath, workPath, projectName, projectVersion, rootPackageEssential: originPackageEssential }) {
