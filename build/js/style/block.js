...
const CLOSE_CHARS = ["}", "]", ")"];
const QUOTE_CHARS = ["`", "'", '"'];
export default function parseBlock(content, blockInArrays = false) {
    content += ";";
                            ...
                            if (key.length > 0) {
                                ...
                                switch (directive) {
                                    case CACHE.ROOT.customAtrules["attach"]:
                                        result.attachment.push(...Use.string.zeroBreaks(value.slice(spaceIndex)));
                                        break;
                                    case CACHE.ROOT.customAtrules["assign"]:
                                        result.assign.push(...Use.string.zeroBreaks(value.slice(spaceIndex)));
                                        break;
                                    default:
