function createWeightedAdjacencyMatrix(sequences) {
    ...
    const formattedNodePairs = new Set(Array.from(nodePairs).map(pair => pair.split(',').map(Number)));
...
function scalarMultiplyAndSumArrays(array1, array2) {
    ...
    return array1.reduce((accumulator, value, index) => accumulator += value * array2[index], 0);
}
...
function sortByRefernce(arrayA, referB) {
    ...
    const combined = arrayA.map((value, index) => ({ value, key: referB[index] }));
...
function execute(sequences, A, B, expression) {
    ...
    const distResult = applyDistributionToArray(matrix, A, nodes.length, B, expression)
    const result = sortByRefernce(nodes, distResult)
    const score = bechmarkSequence(result, Array.from(nodePairs)) / nodePairs.size
