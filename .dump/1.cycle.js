...
function watchClassGroup(arr1, arr2) {
  ...
  mergedSet.forEach((element) => {
    if (arr2.includes(element)) {
      const occurrences = arr2
        .map((value, index) => (value === element ? index : -1))
...
function createCycleBreakRule(array) {
  ...
  for (let index1 = 0; index1 < arrayx.length; index1++) {
    ...
    try {
      ...
      console.error(
        `Error in superImposition at column ${index1}: ${e.message}`,
      );
      continue;
    }
    ...
    for (let index2 = 0; index2 < arr.length; index2++) {
      ...
      const score =
        pos.length > 0 ? pos.reduce((sum, v) => sum + dependents[v], 0) : 0;
      if (score > entry["$"]) {
        entry["#"] = index1;
        entry["$"] = score;
      }
