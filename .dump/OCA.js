function optimalOrder(
  ...
  const P = Array.from({ length: V }, (_, i) => i).sort(
    (a, b) => DModulus[a] - DModulus[b],
  );
  ...
  const Q = Array.from({ length: V }, (_, i) => i).sort(
    (a, b) => DRefer[b] - DRefer[a],
  );
...
// Example usage
const sequences = generate2dIntegerArray(500);
