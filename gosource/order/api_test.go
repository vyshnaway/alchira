...
func TestOrderPreview(t *testing.T) {
	...
	result, err := order.Order(sequences, "preview", "", artifact)
...
// 	OrderAsync(sequences, "preview", "", artifact, func(resp *OrderResponse, err error) {
// 		asyncResult = resp
// 		asyncErr = err
// 		done <- true
// 	})
...
func BenchmarkOrderPreview(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := order.Order(sequences, "preview", "", artifact)
		if err != nil {
