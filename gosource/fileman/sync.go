...
func Sync_File (url, path string) (string, error) {
	status, latestData, err := Read_File(url, true)
	if err == nil && status {
	...
	status, currentData, err := Read_File(path, false)
	if err == nil && status {
...
func Sync_Json (url, path string) (map[string]interface{}, error) {
	status, latestData, err := Read_Json(url, true)
	if err == nil && status {
	...
	// If online fetch fails, try reading local file
	status, currentData, err := Read_Json(path, false)
	if err == nil && status {
		return currentData, nil
	}
...
func Sync_Bulk (source, target string, extInclude, extnUnsync, fileExcludes []string) (map[string]string, error) {
	...
	sourceExists := Path_IfFolder(source)
	targetExists := Path_IfFolder(target)
	...
	for _, relFile := range relativeTargetFiles {
		...
		isUnsyncable := helper_Contains(extnUnsync, filepath.Ext(relFile))
