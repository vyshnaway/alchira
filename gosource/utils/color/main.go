...
func hslToRgb(h, s, l, alpha float64) ColorResult {
	s /= 100
	l /= 100
...
func rgbToHsl(r, g, b, alpha float64) ColorResult {
	r /= 255
	g /= 255
	b /= 255
...
func labToRgb(L, a, b, alpha float64) ColorResult {
	const D65_Xn = 95.047
	const D65_Yn = 100.0
...
// r, g, b: Red, Green, Blue components (0-255), alpha: Opacity (0-1).
func rgbToLab(r, g, b, alpha float64) ColorResult {
	r /= 255
	g /= 255
	b /= 255
...
func lchToRgb(L, C, H, alpha float64) ColorResult {
	hRad := H * math.Pi / 180
	_a := C * math.Cos(hRad)
	_b := C * math.Sin(hRad)
	rgb := labToRgb(L, _a, _b, alpha)
	a_val := math.Max(0, math.Min(1, alpha))
...
// r, g, b: Red, Green, Blue components (0-255), alpha: Opacity (0-1).
func rgbToLch(r, g, b, alpha float64) ColorResult {
	lab := rgbToLab(r, g, b, alpha)
	L := lab.L
	_a := lab.A
	bComp := lab.BComp
...
func hwbToRgb(h, w, bl, alpha float64) ColorResult {
	h = math.Mod(h, 360)
	w /= 100
	bl /= 100

	baseRgb := hslToRgb(h, 100, 50, 1) // HSL to RGB for base hue color
...
// r, g, b: Red, Green, Blue components (0-255), alpha: Opacity (0-1).
func rgbToHwb(r, g, b, alpha float64) ColorResult {
	r /= 255
	g /= 255
	b /= 255
...
// r, g, b: Red, Green, Blue components (0-255), alpha: Opacity (0-1).
func rgbToHex(r, g, b, alpha float64) string {
...
func hexToRgb(hex string) (ColorResult, error) {
...
func oklabToRgb(L, a, b, alpha float64) ColorResult {
...
// r, g, b: Red, Green, Blue components (0-255), alpha: Opacity (0-1).
func rgbToOklab(r, g, b, alpha float64) ColorResult {
...
func oklchToRgb(L, C, H, alpha float64) ColorResult {
	hRad := H * math.Pi / 180
	_a := C * math.Cos(hRad)
	_b := C * math.Sin(hRad)
	rgb := oklabToRgb(L, _a, _b, alpha)
	a_val := math.Max(0, math.Min(1, alpha))
...
// r, g, b: Red, Green, Blue components (0-255), alpha: Opacity (0-1).
func rgbToOklch(r, g, b, alpha float64) ColorResult {
	oklab := rgbToOklab(r, g, b, alpha)
	L := oklab.L
	_a := oklab.A
	_b := oklab.BComp
...
// SwitchRGB provides a structured way to access color conversion functions.
type SwitchRGB struct {
	From struct {
...
// NewSwitchRGB initializes and returns a SwitchRGB struct with all conversion functions.
func New_SwitchRGB() *SwitchRGB {
	s := &SwitchRGB{}
	s.From.Lch = lchToRgb
	s.From.Lab = labToRgb
	s.From.Hwb = hwbToRgb
	s.From.Hsl = hslToRgb
	s.From.Oklch = oklchToRgb
	s.From.Oklab = oklabToRgb

	s.To.Lch = rgbToLch
	s.To.Lab = rgbToLab
	s.To.Hwb = rgbToHwb
	s.To.Hsl = rgbToHsl
	s.To.Oklch = rgbToOklch
	s.To.Oklab = rgbToOklab
