...
func Artifact(index int) artifact_return {
	...
	isPublic := _strings_.Contains(style.SymClass, "$$$")
	element := ""
	...
	if len(style.SnippetStaple) > 0 {
		...
		innertext = _strings_.Join(artifact_Partial(style.StyleObject, true), "")
		element = "style"
	}
	...
	if isPublic {
		for k, v := range style.StyleObject {
			...
			if v_ok {
				stylesheet = append(stylesheet, [2]string{k, _strings_.Join(artifact_Partial(v_typed, true), "")})
			}
		}
	} else {
		stylesheet = [][2]string{{"", ""}}
