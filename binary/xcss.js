  ...
  const adaptedExports = Object.setPrototypeOf(
    {
      runCommand(command, param) {
        // src.as/index/runCommand(~lib/string/String, ~lib/string/String | null) => ~lib/string/String
        command = __retain(__lowerString(command) || __notnull());
        param = __lowerString(param);
        try {
          return __liftString(exports.runCommand(command, param) >>> 0);
        } finally {
          __release(command);
        }
      },
...
export const { memory, runCommand } = await (async (url) =>
  instantiate(
    await (async () => {
      const isNodeOrBun =
    ...
    {},
  ))(new URL("xcss.wasm", import.meta.url));
