import getAppData from './Data/app.js';
import getNavData from './Data/nav.js';
import getRootData from './Data/root.js';
import $ from './Shell/index.js';
import EXECUTOR from './executor.js';
import ACTION from './actor.js'
import fileman, { CSSImport } from '../interface/files.js';
...
const execute = async (isDev = false, backRows = 0) => {
    ...
    if (verified.status) {
        ...
        if (isDev) {
            ...
            const heading = $.compose.primary.Chapter(`Active Runtime : ${nows}`)
...
const commander = async (cmd, arg, rootPath, consoleWidth, packageJson) => {
    ...
    Object.assign(PACKAGE, packageJson)
    Object.assign(ROOT, getRootData(rootPath, APP.content))
    Object.assign(APP, getAppData(packageJson))
    Object.assign(NAV, getNavData(rootPath))
    ...
    switch (DATA.CMD) {
        ...
        default:
            ...
            $.WRITE.std.Chapter(`${APP.command} @ ` + APP.version, [ROOT.DOCS.alerts.content])
            ...
            $.WRITE.secondary.Section("Documentation : " + ROOT.DOCS.readme.path,
