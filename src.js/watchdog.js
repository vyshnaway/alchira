...
export default function WATCHDOG(directories, onChange) {
    if (!Array.isArray(directories) || directories.length === 0 || typeof onChange !== 'function') {
        ...
        setInterval(() => {
            fs.readdir(dir, { withFileTypes: true }, (err, files) => {
                ...
                Object.keys(lastStats).forEach(file => {
                    if (!currentStats[file]) onChange('delete', file);
                });
                Object.keys(currentStats).forEach(file => {
                    if (!lastStats[file]) onChange('add', file);
                    else if (lastStats[file] !== currentStats[file]) onChange('change', file);
                });
            ...
            fs.readdir(dir, { withFileTypes: true }, (err, files) => {
                ...
                files.forEach(file => {
                    if (file.isDirectory()) {
                        ...
                        if (!resolvedDirs.includes(subDir)) WATCHDOG([subDir], onChange);
                    }
                });
            });
        }, 1000);
